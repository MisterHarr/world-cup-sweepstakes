 ...
 * back.  Cost: 1 read + 1 write per ingest cycle instead of N × 104 client
 * reads across all connected users.
 *
 * teamNames / teamFlags in the summary are seeded by adminSeedMatchSummary
 * and are not touched here (they only change when teams are re-seeded).
 */
async function patchMatchSummary(
  db: FirebaseFirestore.Firestore,
